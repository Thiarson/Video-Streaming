...
const FlvSession = require('./flv-session');
...
class RtmpSession {
  ...
  async onPublish(invokeMessage) {
    ...
    if (this.config.auth && this.config.auth.publish && !this.isLocal) {
      let results = CoreUtils.verifyAuth(this.publishArgs.sign, this.publishStreamPath, this.config.auth.secret);
      if (!results) {
  ...
  onPlay(invokeMessage) {
    ...
    if (this.config.auth && this.config.auth.play && !this.isLocal) {
      let results = CoreUtils.verifyAuth(this.playArgs.sign, this.playStreamPath, this.config.auth.secret);
      if (!results) {
